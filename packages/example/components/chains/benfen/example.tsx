    ...
    <>
      ...
      <ApiGroup title="Transaction">
        ...
        <ApiForm title="Benfen Swap" description="Benfen Swap 相关操作">
          ...
          <ApiForm.Checkbox id="useBusdGas" label="使用 BUSD 作为 gas" />
          ...
          <ApiForm.Button
            ...
            onClick={async (formRef) => {
              ...
              const busdGas = formRef?.getValue<boolean>('useBusdGas');
          ...
          <ApiForm.Button
            ...
            onClick={async (formRef) => {
              ...
              let coin: ReturnType<(typeof tx)['splitCoins']>;
              // if(busdGas) {
              //   const totalBalance = busdCoins.reduce((sum, coin) => sum + BigInt(coin.balance), BigInt(0));
              //   const gasAmount = totalBalance - bigintAmount;
              //   // 先从合并后的 coin 分割出用于交换的部分
              //   const [coin1,coin2] = tx.splitCoins(tx.object(primaryCoin.coinObjectId), [tx.pure(bigintAmount),tx.pure(gasAmount)]);
              //   coin = coin1 as unknown as ReturnType<(typeof tx)['splitCoins']>;
              //   gasCoins = [coin2 as unknown as CoinStruct];
              // } else {
