...
generator is deterministic: it requires a unique recorded locator for each retained action,
stops at the repository-icon outcome, adds the standard readiness action, and writes a temporary
candidate. It atomically replaces `e2e.mjs` only after syntax validation and a successful real
Desktop attempt with a fresh non-persistent WebView and exact repository-icon detection. It tries
at most five fresh sessions and stops after the first success. Every failed generation preserves
the previous canonical script.
...
partition and a new WebView target in the real OneKey Desktop DApp Browser.
