...
import {
  applyCustomProtocolPatch,
  validateCustomProtocolRegistry,
} from '../lib/custom-protocol-registry.mjs';
import {
  applyProtocolPatch,
  saveRegistry,
  validateRegistry,
...
function makePatch(args, now) {
  ...
  if (action === 'set-review') {
    ...
    throw new Error('--state must be pending, processed, or unsupported');
  }
...
try {
  ...
  const protocol = isCustomRegistry
    ? applyCustomProtocolPatch(registry, protocolId, makePatch(args, now))
    : await applyProtocolPatch(registry, protocolId, makePatch(args, now));
  const errors = isCustomRegistry
    ? validateCustomProtocolRegistry(registry)
